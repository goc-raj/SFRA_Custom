...
server.replace(
    ...
    function (req, res, next) {
        ...
        if ('useGiftCard' in currentBasket.custom && currentBasket.custom.useGiftCard === "no") {
            paymentMethodIdValue = "GIFT_CERTIFICATE";
            viewData.gcCode = currentBasket.custom.gcCode;
        } else {
        ...
        this.on('route:BeforeComplete', function (req, res) { // eslint-disable-line no-shadow
            ...
            if (paymentMethodID != 'GIFT_CERTIFICATE' && 'gcCode' in currentBasket.custom && currentBasket.custom.gcCode) {
                var updatePaymentTransaction = COHelpers.updatePaymentTransaction(currentBasket);
                if(!updatePaymentTransaction.error) {
                    basketCalculationHelpers.calculateTotals(currentBasket);
                    var statusCodes = {
                        'STATUS_PENDING': 0,
                        'STATUS_ISSUED': 1,
                        'STATUS_PARTIALLY_REDEEMED': 2,
                        'STATUS_REDEEMED': 3
                    };
                    Transaction.begin();
                    var giftCert = GiftCertificateMgr.getGiftCertificateByCode(currentBasket.custom.gcCode);
                    giftCert.setStatus(statusCodes['STATUS_REDEEMED']);
                    Transaction.commit();
                } else {
                    res.json({
                        form: paymentForm,
